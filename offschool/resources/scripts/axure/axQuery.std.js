...
//            var wasShown = $ax.visibility.IsIdVisible(elementId);
    ...
    $ax.public.fn.enabled = function() {
        if(arguments[0] == undefined) {
            ...
            else return this.jQuery().children(':disabled').length <= 0 2 0; } else { var elementids="this.getElementIds();" for(var index="0;" < elementids.length; index++) elementid="elementIds[index];" widgettype="$ax.getTypeFromElementId(elementId);" enabled="arguments[0];" if ($ax.public.fn.isimagebox(widgettype) || $ax.public.fn.isvector(widgettype) $ax.public.fn.istextbox(widgettype) $ax.public.fn.istextarea(widgettype) $ax.public.fn.iscombobox(widgettype) $ax.public.fn.islistbox(widgettype) $ax.public.fn.ischeckbox(widgettype) $ax.public.fn.isradiobutton(widgettype) ) $ax.style.setwidgetenabled(elementid, enabled); ($ax.public.fn.isdynamicpanel(widgettype) $ax.public.fn.islayer(widgettype)) children="this.getChildren(false," true)[index].children; i="0;" children.length; i++) $axure('#' + children[i]).enabled(enabled); obj="$obj(elementId);" images="obj.images;" if(plain_text_types.indexof(widgettype) !="-1" && images) img="$jobj($ax.repeater.applySuffixToElementId(elementId," '_image_sketch')); key="(enabled" ? 'normal~' : 'disabled~') ($ax.adaptive.currentviewid ''); img.attr('src', images[key]); jobj="$jobj(elementId);" input="$jobj($ax.INPUT(elementId));" if(input.length) (os_mac webkit $ax.public.fn.iscombobox(widgettype)) jobj.css('color', '' 'graytext'); if(enabled) jobj.prop('disabled', false); true); return this; }; $ax.public.fn.visible="function()" ids="this.getElementIds();" ids.length; $ax.visibility.setidvisible(ids[index], arguments[0]); $ax.public.fn.selected="function()" if(arguments[0]="=" undefined) firstid="this.getElementIds()[0];" if(!firstid) undefined; ($ax.public.fn.istreenodeobject(widgettype)) treenodebuttonshapeid ; allelementids="$ax.getAllElementIds();" allelementids.length; currobj="$ax.getObjectFromElementId(elementId);" ($ax.public.fn.isvector(currobj.type) currobj.parent currobj.parent.scriptids currobj.parent.scriptids[0]="=" firstid) break; if(treenodebuttonshapeid="=" '') $ax.style.iswidgetselected(treenodebuttonshapeid); $ax.public.fn.istablecell(widgettype) $ax.public.fn.isdynamicpanel(widgettype) $ax.style.iswidgetselected(firstid); ($ax.public.fn.ischeckbox(widgettype) $ax.public.fn.isradiobutton(widgettype)) $jobj($ax.input(firstid)).prop('checked'); func="typeof" (arguments[0])="==" 'function' arguments[0] null; this is a function it will be overridden with the value; if(func) elementid)); for tree node treerootid="$('#'" elementids[index]).parents('.treeroot').attr('id'); childelementids="$jobj(elementId).children();" childelementids.length; (currobj currobj.type="=" $ax.constants.vector_shape_type elementids[index]) continue; $ax.tree.selecttreenode(elementid, $ax.style.setwidgetselected(elementids[index], query="$jobj($ax.INPUT(elementId));" curr="query.prop('checked');" note: won't fire onselect nore onunselect event states didn't changes if(curr query.prop('checked', enabled, $ax.event.tryfirecheckchanged(elementid, $ax.public.fn.focus="function()" focusableid="$ax.event.getFocusableWidgetOrChildId(firstId);" scroll but not focus $('#' focusableid).triggerhandler("focus"); does call our custom so animate focusableid).focus(); $ax.public.fn.expanded="function()" !$ax.public.fn.istreenodeobject($ax.gettypefromelementid(firstid)) $ax.visibility.isidvisible(firstid '_children'); ($ax.public.fn.istreenodeobject($ax.gettypefromelementid(elementids[index]))) treenodeid="elementIds[index];" childcontainerid="treeNodeId" '_children'; scriptid="$ax.repeater.getScriptIdFromElementId(treeNodeId);" itemid="$ax.repeater.getItemIdFromElementId(treeNodeId);" plusminusid="u" (parseint(scriptid.substring(1)) 1); if(itemid) itemid); if($('#' childcontainerid).length="=" !$jobj(plusminusid).children().first().is('img')) true) $ax.tree.expandnode(treenodeid, childcontainerid, plusminusid); false) $ax.tree.collapsenode(treenodeid, _populateboundingrect="function" (boundingrect) boundingrect.right="boundingRect.left" boundingrect.width; boundingrect.bottom="boundingRect.top" boundingrect.height; boundingrect.x="boundingRect.left;" boundingrect.y="boundingRect.top;" boundingrect.location="{" x: boundingrect.left, y: boundingrect.top, left: top: boundingrect.top boundingrect.size="{" width: boundingrect.width, height: boundingrect.height boundingrect.centerpoint="{" boundingrect.width boundingrect; boundingrect relative to its offset parent _getboundingrectforsinglewidget="function" (elementid) element="document.getElementById(elementId);" tempboundingrect, position; state="$ax.style.generateState(elementId);" style="$ax.style.computeFullStyle(elementId," state, $ax.adaptive.currentviewid); position="{" style.location.x, style.location.y tempboundingrect="{" style.location.y, style.size.width, style.size.height ($ax.public.fn.iscompoundvectorhtml(element)) tempboundingrect.width="Number(element.getAttribute('data-width'));" tempboundingrect.height="Number(element.getAttribute('data-height'));" boundingelement="element;" ($ax.dynamicpanelmanager.isidfittocontent(elementid)) stateid="$ax.visibility.GetPanelState(elementId);" (stateid from getloc fixed="_fixedOffset(id," vert); (fixed.valid) loc="!vert" fixed.fullwidth fixed.offset; jelement="$(element);" (jelement.css('position')="=" 'fixed') position.left "")); position.top position.left, position.top, tempboundingrect.width, _populateboundingrect(boundingrect); _getboundingrectformultiplewidgets="function" (widgetsidarray) (!widgetsidarray widgetsidarray.constructor array) (widgetsidarray.length="=" 0) 0, centerpoint: }, widgetrect="_getBoundingRectForSingleWidget(widgetsIdArray[0]);" widgetrect.left, right: widgetrect.right, widgetrect.top, bottom: widgetrect.bottom (var widgetsidarray.length; boundingrect.left="Math.min(boundingRect.left," widgetrect.left); widgetrect.top); widgetrect.right); widgetrect.bottom); (boundingrect.right boundingrect.left) 2.0, (boundingrect.bottom boundingrect.top) 2.0 - boundingrect.left; boundingrect.top; _getlayerchildrendeep="$ax.public.fn.getLayerChildrenDeep" = (layerid, includelayers, includehidden) deep="[];" layerid).getchildren()[0].children; childid="children[index];" (!includehidden !$ax.visibility.isidvisible(childid)) ($ax.public.fn.islayer($obj(childid).type)) (includelayers) deep.push(childid); recursivechildren="_getLayerChildrenDeep(childId," includehidden); j="0;" recursivechildren.length; j++) deep.push(recursivechildren[j]); deep; _boundingrectforids="function(childIds)" default size childrenboundingrect="{" childids.length> 0 ? 9999999 : 0, top: childIds.length > 0 ? 9999999 : 0, right: 0, bottom: 0 };
