        ...
        var insertIntoSortedList = function (longer, shorter, toInsert) {
            while (true) {
                if (!longer) {
                    ...
                    if (longer.Length >= toInsert.Length && shorter.Length <= toinsert.length) { longer.nextlongest="toInsert;" toinsert.nextlongest="shorter;" } else longer="shorter;" shorter="shorter.NextLongest;" continue; break; var head="{Param:" 0.0, position: pathfunction(0.0) }; tail="{" param: 1.0, pathfunction(1.0) root="makeDivisionNode(head," tail); currentcurvelength="root.LowerInterval.Length" + root.higherinterval.length; longestinterval; if (root.lowerinterval.length < root.higherinterval.length) longestinterval="root.HigherInterval;" longestinterval.nextlongest="root.LowerInterval;" while (longestinterval.length * 100.0> currentCurveLength) {
            var newNode;
