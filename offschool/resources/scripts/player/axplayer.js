...
(function () {
    ...
    function appendOutOfDateNotification() {
        ...
        if (!MOBILE_DEVICE) {
            toAppend += '       <div style="margin-right: 36px"><a href="http://outdatedbrowser.com/en" id="updateBrowserButton" target="_blank" rel="noopener">UPDATE BROWSER</a></div>';
    ...
    function calculateScrollLeftWithOffset(offset, isLeftPanel) {
        ...
        if (isCentered) {
            ...
            if (clippedContentWidth <= 4 200 0) { viewadjustment="offset" 2; } else if (iscollapsing && clippedcontentwidth < offset) - clippedcontentwidth) (!iscollapsing -offset) + return leftpos viewadjustment; set to true when left panel or right are being expanded collapsed returns false lsplitbar (switched clippingbounds) finishes animation (thus panels will be fully retracted at this point) var isanimating="$axure.player.isAnimating" = false; $axure.player.collapsetobar="function" (context, hostid) lastleftpanelwidth="$('.leftPanel').width();" lastrightpanelwidth="$('.rightPanel').width();" (context="==" 'project' || context="==" 'all') if(!ismobilemode()) newwidth="lastLeftPanelWidth" !="0" ? : lastleftpanelwidthdefault; clippingwidth="calculateClippingBoundsWidth(0," true); newleft="calculateScrollLeftWithOffset(newWidth," $('.leftpanel').animate({ 'margin-left': -newwidth 'px' }, duration: 200, complete: function() $('.leftpanel').width(0).hide().css({ '' }); $('#lsplitbar').animate({ left: '-4px' $('#lsplitbar').hide(); $('#clippingbounds').animate({ '', width: $('#clippingboundsscrollcontainer').animate({ function () $axure.player.resizecontent(); $axure.player.pluginvisiblechanged(hostid, false); }}); $('.leftpanel').width(0); 'inspect' (!ismobilemode()) lastrightpanelwidthdefault; $('.rightpanel').animate({ 'margin-right': $('.rightpanel').width(0).hide().css({ $('#rsplitbar').animate({ $(window).width() $('#rsplitbar').hide(); $('.rightpanel').width(0); $(window).resize(); toolbaronly="true;" $axure.player.expandfrombar="function" (hostid, context, isfinalplugintorestore) 'project') ($('#lsplitbar').is(':visible')) return; $('.leftpanel').removeclass('popup'); lastleftpanelwidthdefault); $('.leftpanel').width(newwidth); $('.leftpanel').css('margin-left', 'px').show(); '0px' $('.leftpanel').css({ $('#lsplitbar').css('left', '-4px'); $('#lsplitbar').show(); 'px', (isfinalplugintorestore) $('#clippingboundsscrollcontainer').show(); if($('#rsplitbar').is(':visible')) $('#' hostid).show(); $('.rightpanel').width(newwidth); $('.rightpanel').css('margin-right', 'px'); $('.rightpanel').css({ $('#rsplitbar').css('left', $(window).width()); $('#rsplitbar').show(); $('.rightpanel').width() (ismobilemode()) $('#mhidesidebar').show(); $('#nativeappcontrolframe').show(); suspendrefreshviewport="$axure.player.suspendRefreshViewPort" $axure.player.refreshviewport="function" (suspendrefreshviewport) dimstr="$('.currentAdaptiveView').attr('data-dim');" dim="dimStr" dimstr.split('x') w: '0', h: '0' }; w="dim[0]" dim[0] ''; h="dim[1]" dim[1] scaleval="$('.vpScaleOption').find('.selectedRadioButton').parent().attr('val');" $axure.player.noframe="false;" (h 1) cliptoview="h" !$axure.player.noframe; isdevice="h;" mainpanelwidth="$('#mainPanel').width();" mainpanelheight="$('#mainPanel').height();" (!w !cliptoview) (!h (mobile_device> mainPanelHeight) h = mainPanelHeight;
                    ...
                    document.body.addEventListener('touchmove', function (event) {
                        ...
                        if (!owner) {
                            ...
                            if ($(owner).scrollTop() + $(owner).height() == owner.scrollHeight) {
                                if (touching) {
                                    if (event.pageY <= 414 1125 pageystart) { event.preventdefault(); } }, passive: false }); document.body.addeventlistener('touchstart', function (event) var owner="getScrollOwner(event.target);" if (owner) ($(owner).scrolltop()="=" 0) touching="true;" pageystart="event.pageY;" pageyoffset="event.pageY;" + $(owner).height()="=" owner.scrollheight) removeelasticscrollfromiframe(); $('html').css('-webkit-tap-highlight-color', 'transparent'); stop ios from automatically scaling parts of the mobile player could automatic on ipads as well that we actually want, but for now, seems fine $('body').css('-webkit-text-size-adjust', '100%'); prepare iphone x hacks link dimensions: https: kapeli.com cheat_sheets ios_design.docset contents resources documents index ratio="window.devicePixelRatio" || 1; regular iphonex (ios && window.screen.width * window.screen.height 2436) xs max and xr 896) window.addeventlistener("orientationchange", () viewport="document.querySelector("meta[name=viewport]");" so doesn't zoom when switching back to portrait (iphonex) viewport.setattribute('content', 'width="device-width," initial-scale="1.0," maximum-scale="1.0," viewport-fit="cover');" else $axure.player.resizecontent(); false); $axure.page.bind('load.start', always append both browser menu native menu, app might not have returned value signifying at this point appendnativeprototypecontrolframe(); appendmobilebrowsercontrolframe(); appendprojectoptions(); initializelogo(); initializepreview(); $axure.player.resizecontent(true); has timeout keep waiting build sign in controls while axaccount is still loading initializesignin(); })(); appendprojectoptions() toappend ; $('#mhidesidebar').prepend(toappend); $(('#projectoptionshost')).click(function (e) e.stoppropagation(); (ismobilemode()) appendmobilebrowsercontrolframe() $('#mobilecontrolframecontainer').append(toappend); $('#closebackground').click(collapse); will do incorrect click position content bounds calculation which results scroll getting reset (0, (ios) $('#mobilecontrolframecontainer').on($axure.eventnames.mousedownname, appendnativeprototypecontrolframe() barheight="IOS" ? (iphonex '82px' : '72px') '60px'; returnicondisplacement="IOS" '-15px': '-20px'; icontopmargin="IOS" '14px': '7px'; returntexttopmargin="IOS" '9px': document.getelementbyid('nativeappcontrolframe').style.height="barHeight;" document.getelementbyid('nativemenubluebackground').style.height="barHeight;" document.getelementbyid('return').style.top="returnIconDisplacement;" document.getelementbyid('returntext').style.margintop="returnTextTopMargin;" document.getelementbyid('refreshicon').style.margintop="iconTopMargin;" document.getelementbyid('exiticon').style.margintop="iconTopMargin;" addappbuttonclicklistener("exit"); addappbuttonclicklistener("refresh"); $('#returnbackground').click(collapse); $('#nativeappcontrolframe').on('touchmove', addappbuttonclicklistener(id) func="function" window.webkit.messagehandlers.prototypemenubuttonclick.postmessage(id) shareapp.prototypemenubuttonclick(id); }; document.getelementbyid(id).addeventlistener("click", func, togglesitemap() $axure.player.showplugin(1); closepopup() $container="$('.popup');" isleftpanel="$container.hasClass('leftPanel');" $container.removeclass('popup'); $('#overflowmenubutton').removeclass('selected'); $('#interfaceadaptiveviewscontainer').removeclass('selected'); $container.hide(); $('div.splittermask').unbind($axure.eventnames.mousedownname, closepopup); $('div.splittermask').remove(); $axure.player.closepopup="closePopup;" showpopup($container) ($('#browseroutofdatenotification').is(":visible")) return; $container.addclass('popup'); $container.show(); $('<div class="splitterMask">').insertAfter($container);
